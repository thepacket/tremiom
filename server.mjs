...
async function handleEventAutopick(req, res) {
  ...
  const proc = spawn(PYTHON, ['workers/event_autopick.py'], {
    stdio: ['pipe', 'pipe', 'inherit'],
  });
  let stdout = '';
  const timer = setTimeout(() => proc.kill('SIGTERM'), 90_000);
  proc.stdout.on('data', (c) => { stdout += c.toString('utf8'); });
  proc.on('exit', (code) => {
    clearTimeout(timer);
    if (code !== 0 || !stdout) {
      res.writeHead(502, { 'content-type': 'application/json' });
      res.end(JSON.stringify({ error: 'autopick failed', code })); return;
    }
    res.writeHead(200, { 'content-type': 'application/json' });
    res.end(stdout);
  });
...
const httpServer = http.createServer((req, res) => {
  ...
  if (req.url?.startsWith('/api/event/autopick')) {
    handleEventAutopick(req, res);
    return;
  }
  ...
  if (req.url?.startsWith('/api/waveform/panels')) {
    handleWaveformPanels(req, res);
    return;
  }
